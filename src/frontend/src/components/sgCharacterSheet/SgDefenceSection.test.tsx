...
test('Parry is calculated and shown right', async () =>
{
    act(() => {
    ...
    //Best Equipped weapon is used 
    let axe = testCharacter.equipments.find(t => t.equipment.name === "Throwing Axe")!;
    expect(await screen.findByLabelText(SgCharacterSheetAriaLabels.getEquipmentRowLabel(axe))).toBeInTheDocument();

    let parry: IActiveDefenceData = getCharacterParry(testCharacter);
    let parryLabels = SgCharacterSheetAriaLabels.getActiveDefenceDataLabels(parry);
    expect(await screen.findByLabelText(parryLabels.defenceName)).toBeInTheDocument();
    let parryProviderName = await screen.findByLabelText(parryLabels.defenceProviderName);
    expect(parryProviderName.textContent).toBe("Throwing Axe");
    let parryValue = await screen.findByLabelText(parryLabels.value);
    expect(parryValue.textContent).toBe("8");

    let axeEquipmentRow = await screen.findByLabelText(SgCharacterSheetAriaLabels.getEquipmentRowLabel(axe));
    expect(axeEquipmentRow).toBeInTheDocument();
    
    let isAxeEquipped = (await screen.findByLabelText(SgCharacterSheetAriaLabels.getEquipmentIsEquippedCheckBoxLabel(axe))) as HTMLInputElement;
    expect(isAxeEquipped).toBeChecked();
    act(() => {
        userEvent.click(isAxeEquipped);
    })
    expect(isAxeEquipped).not.toBeChecked();
    
    //Second equipped weapon is used
    let sword = testCharacter.equipments.find(t => t.equipment.name === "Thrusting Bastard Sword")!;
    expect(await screen.findByLabelText(parryLabels.defenceName)).toBeInTheDocument();
    parryProviderName = await screen.findByLabelText(parryLabels.defenceProviderName);
    expect(parryProviderName.textContent).toBe("Thrusting Bastard Sword");
    parryValue = await screen.findByLabelText(parryLabels.value);
    expect(parryValue.textContent).toBe("6");

    let isSwordEquipped = (await screen.findByLabelText(SgCharacterSheetAriaLabels.getEquipmentIsEquippedCheckBoxLabel(sword))) as HTMLInputElement;
    expect(isSwordEquipped).toBeChecked();
    act(() => {
        userEvent.click(isSwordEquipped);
    })
    expect(isSwordEquipped).not.toBeChecked();
    
    //Best parry skill is used
    expect(await screen.findByLabelText(parryLabels.defenceName)).toBeInTheDocument();
    let judoProviderName = await screen.findByLabelText(parryLabels.defenceProviderName);
    expect(judoProviderName.textContent).toBe("Judo");
    let judoValue = await screen.findByLabelText(parryLabels.value);
    expect(judoValue.textContent).toBe("9");

    let removeJudoButton = await screen.findByLabelText(
        SgCharacterSheetAriaLabels.getSkillRemoveButtonLabel(testCharacter.skills.find(s => s.skill.name === "Judo")!.skill));
    expect(removeJudoButton).toBeInTheDocument();
    act(() => {
        userEvent.click(removeJudoButton);
    })

    let okButton = await screen.findByLabelText(SgCharacterSheetAriaLabels.CharacterSkillsRemoveDialogOkButton);
    ...
    //Dexterity is used
    expect(await screen.findByLabelText(parryLabels.defenceName)).toBeInTheDocument();
    let dexterityProviderName = await screen.findByLabelText(parryLabels.defenceProviderName);
    expect(dexterityProviderName.textContent).toBe("Dexterity");
    let dexterityValue = await screen.findByLabelText(parryLabels.value);
    expect(dexterityValue.textContent).toBe("9");
})

test('Dodge is calculated and shown right', async () =>
{
    act(() => {
        renderWithProviders(<FrodoCharacterPage/>);
    });
    ...
    let dodge: IActiveDefenceData = getCharacterDodge(testCharacter);
    let dodgeLabels = SgCharacterSheetAriaLabels.getActiveDefenceDataLabels(dodge);
    expect(await screen.findByLabelText(dodgeLabels.defenceName)).toBeInTheDocument();
    let dodgeProviderName = await screen.findByLabelText(dodgeLabels.defenceProviderName);
    expect(dodgeProviderName.textContent).toBe("");
    let dodgeValue = await screen.findByLabelText(dodgeLabels.value);
    expect(dodgeValue.textContent).toBe("9");
    ...
    dodgeProviderName = await screen.findByLabelText(dodgeLabels.defenceProviderName);
    expect(dodgeProviderName.textContent).toBe("");
    dodgeValue = await screen.findByLabelText(dodgeLabels.value);
    expect(dodgeValue.textContent).toBe("8");
})
